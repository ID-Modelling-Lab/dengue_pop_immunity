...
simulate_model = function(parameters){
  ...
  selected_immune_1 = sample(N, floor(immuneList[1]*N)) # gives indices of selected individuals
  ...
  immune_12 = intersect(selected_immune_1, selected_immune_2) # overlapping indices
  ...
  status[immune_12] = sample(c(37,40), length(immune_12), replace = TRUE)
  status[immune_13] = sample(c(38,43), length(immune_13), replace = TRUE)
  status[immune_14] = sample(c(39,46), length(immune_14), replace = TRUE)
  status[immune_23] = sample(c(41,44), length(immune_23), replace = TRUE)
  status[immune_24] = sample(c(42,47), length(immune_24), replace = TRUE)
  status[immune_34] = sample(c(45,48), length(immune_34), replace = TRUE)
  
  # Immune to 3 serotypes
  immune_123 = intersect(immune_12, selected_immune_3)
  immune_124 = intersect(immune_12, selected_immune_4)
  immune_134 = intersect(immune_13, selected_immune_4)
  immune_234 = intersect(immune_23, selected_immune_4)
  
  status[immune_123] = sample(c(97,99,103,105,109,111), length(immune_123), replace = TRUE)
  status[immune_124] = sample(c(98,101,104,107,115,117), length(immune_124), replace = TRUE)
  status[immune_134] = sample(c(100,102,110,113,116,119), length(immune_134), replace = TRUE)
  status[immune_234] = sample(c(106,108,112,114,118,120), length(immune_234), replace = TRUE)
  
  # Immune to 4 serotypes
  immune_1234 = intersect(immune_123, selected_immune_4)
  
  status[immune_1234] = 145
  ...
  # sample from the totally susceptible individuals
  ...
  new_I1 = numeric(time_steps+1)  # new serotype 1 infections
  new_I2 = numeric(time_steps+1)  # new serotype 2 infections
  new_I3 = numeric(time_steps+1)  # new serotype 3 infections
  new_I4 = numeric(time_steps+1)  # new serotype 4 infections
  
  detected_I1 = numeric(time_steps+1) # only primary and secondary cases detected
  detected_I2 = numeric(time_steps+1) # only primary and secondary cases detected
  detected_I3 = numeric(time_steps+1) # only primary and secondary cases detected
  detected_I4 = numeric(time_steps+1) # only primary and secondary cases detected
  ...
  out = tryCatch({
    for(tt in 1:(time_steps+1)) {
      ...
      curr_I1_ps = sum(status == 1 | status == 16 | status == 19 | status == 22)
      curr_I2_ps = sum(status == 2 | status == 13 | status == 20 | status == 23)
      curr_I3_ps = sum(status == 3 | status == 14 | status == 17 | status == 24)
      curr_I4_ps = sum(status == 4 | status == 15 | status == 18 | status == 21)
      ...
      new_I1[tt] =  sum(status == 1 | status == 16 | status == 19 | status == 22 |
                          status == 57 | status == 59 | status == 63 | status == 65 | status == 69 | status == 71 |
                          status == 130 | status == 132 | status == 136 | status == 138 | status == 142 | status == 144) - curr_I1
      
      new_I2[tt] = sum(status == 2 | status == 13 | status == 20 | status == 23 |
                         status == 51 | status == 53 | status == 61 | status == 66 | status == 67 | status == 72 |
                         status == 124 | status == 126 | status == 134 | status == 137 | status == 140 | status == 143) - curr_I2
      
      new_I3[tt] = sum(status == 3 | status == 14 | status == 17 | status == 24 |
                         status == 49 | status == 54 | status == 55 | status == 60 | status == 68 | status == 70 |
                         status == 122 | status == 125 | status == 128 | status == 131 | status == 139 | status == 141) - curr_I3
      
      new_I4[tt] = sum(status == 4 | status == 15 | status == 18 | status == 21 |
                         status == 50 | status == 52 | status == 56 | status == 58 | status == 62 | status == 64 |
                         status == 121 | status == 123 | status == 127 | status == 129 | status == 133 | status == 135) - curr_I4
      
      detected_I1[tt] = sum(status == 1 | status == 16 | status == 19 | status == 22) - curr_I1_ps
      detected_I2[tt] = sum(status == 2 | status == 13 | status == 20 | status == 23) - curr_I2_ps
      detected_I3[tt] = sum(status == 3 | status == 14 | status == 17 | status == 24) - curr_I3_ps
      detected_I4[tt] = sum(status == 4 | status == 15 | status == 18 | status == 21) - curr_I4_ps
  ...
  res = data.frame(time = 0:time_steps,
                   ...
                   new.I3 = new_I3,
                   new.I4 = new_I4,
                   detected.I1 = detected_I1,
                   detected.I2 = detected_I2,
                   detected.I3 = detected_I3,
                   detected.I4 = detected_I4
  )
