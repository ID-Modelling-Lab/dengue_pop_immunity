...
simulate_model = function(parameters){
  ...
  serotypeCross = unlist(list.map(N_parameters, serotypeCross), use.names=FALSE)
  ...
  lapply(1:11, function(ageGrp) {
    ...
    # assigning cross-reaction
    selected_immune_12 = sample(age_status_index, floor(length(age_status_index) * immuneList[ageGrp,5]))
    immune_alloc = rbinom(length(selected_immune_12), 1, 0.5)
    immune_alloc[immune_alloc == 1] = 10
    immune_alloc[immune_alloc == 0] = 9
    ...
    immune_alloc = rbinom(length(selected_immune_13), 1, 0.5)
    immune_alloc[immune_alloc == 1] = 11
    immune_alloc[immune_alloc == 0] = 9
    ...
    immune_alloc = rbinom(length(selected_immune_14), 1, 0.5)
    immune_alloc[immune_alloc == 1] = 12
    immune_alloc[immune_alloc == 0] = 9
    ...
    immune_alloc = rbinom(length(selected_immune_24), 1, 0.5)
    immune_alloc[immune_alloc == 1] = 12
    immune_alloc[immune_alloc == 0] = 10
    status[selected_immune_24] <<- immune_alloc
    
    # assigning secondary
    selected_immune_sec = sample(age_status_index, floor(length(age_status_index) * immuneList[ageGrp,9]))
    status[selected_immune_sec] <<- 25
    ...
    serotypeCross[selected_immune_12] <<- 1
    serotypeCross[selected_immune_13] <<- 2
    serotypeCross[selected_immune_14] <<- 3
    serotypeCross[selected_immune_24] <<- 4
  })
  ...
  out = tryCatch({
    for(tt in 1:(time_steps+1)) {
      ...
      lapply(1:11, function(ageGrp) {
        curr_I1[[ageGrp]] <<- sum((status == 1 | status == 16 | status == 19 | status == 22) & ageGroup == ageGrp)
        curr_I2[[ageGrp]] <<- sum((status == 2 | status == 13 | status == 20 | status == 23) & ageGroup == ageGrp)
        curr_I3[[ageGrp]] <<- sum((status == 3 | status == 14 | status == 17 | status == 24) & ageGroup == ageGrp)
        curr_I4[[ageGrp]] <<- sum((status == 4 | status == 15 | status == 18 | status == 21) & ageGroup == ageGrp)
      })
      ...
      lapply(1:11, function(ageGrp) {
        newI1[[ageGrp]][tt] <<- sum((status == 1 | status == 16 | status == 19 | status == 22) & ageGroup == ageGrp) - curr_I1[[ageGrp]]
        newI2[[ageGrp]][tt] <<- sum((status == 2 | status == 13 | status == 20 | status == 23) & ageGroup == ageGrp) - curr_I2[[ageGrp]]
        newI3[[ageGrp]][tt] <<- sum((status == 3 | status == 14 | status == 17 | status == 24) & ageGroup == ageGrp) - curr_I3[[ageGrp]]
        newI4[[ageGrp]][tt] <<- sum((status == 4 | status == 15 | status == 18 | status == 21) & ageGroup == ageGrp) - curr_I4[[ageGrp]]
      })
  ...
  output = list(
    ...
    # "status" =  status,
    "times" = c(a),
    "serotypes" = list(serotype1, serotype2, serotype3, serotype4, serotypeCross)
  )
